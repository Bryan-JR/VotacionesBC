<template>
  <v-card class="mx-auto pa-0 bg-teal-lighten-5 my-8" max-width="380">
    ...
    <h2 class="font-weight-regular text-center pa-4">{{ candidato.programa1 }} - {{ candidato.programa2 }}</h2>

    <v-card-actions class="d-flex justify-center">
      <v-btn class="bg-lime-darken-3" v-if="!voted[0]" @click="votar(candidato.idCandidato)">Votar</v-btn>
      <v-chip color="primary" v-if="voted[0]&&candidato.idCandidato===votado.id"> Votado </v-chip>
      <br>
      <v-chip  v-if="user.type==='admin'"> Votos: {{ datos.votos }} </v-chip>
    </v-card-actions>
  </v-card>
</template>
